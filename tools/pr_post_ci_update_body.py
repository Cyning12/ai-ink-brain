#!/usr/bin/env python3
"""PR 全绿后更新 body：CI 状态表、Test plan 勾选、变更范围统计。

前端仓镜像 · 语义见配对后端 `SPEC-Governance-PR-Post-CI-v1.md`。
"""
...
# 与 branch protection / quality.yml · verify-fast.yml job 名对齐
...
# automerge 路径闸（见 .mergify.yml · 勿与 docs-only 白名单冲突）
...
def render_scope_table(files: list[str]) -> str:
    ...
    if blocked:
        lines.extend(
            [
                "",
                "**⚠ automerge 路径闸**：含 `app/`、`lib/`、`.github/workflows/`、`package.json` 等 — "
