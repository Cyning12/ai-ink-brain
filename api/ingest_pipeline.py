"""
content/ → 分块 → SiliconFlow 向量 → Supabase documents（与 lib/ingest.ts 行为对齐）。
"""
...
def chunk_text_by_chars(text: str, chunk_size: int = CHUNK_SIZE, overlap: int = CHUNK_OVERLAP) -> list[tuple[str, int]]:
...
def get_all_markdown_chunks() -> list[IngestChunk]:
    ...
    for abs_p, rel in _walk_markdown(content_root):
        stat = abs_p.stat()
        last_modified = datetime.fromtimestamp(stat.st_mtime, tz=timezone.utc).isoformat().replace(
            "+00:00", "Z"
        )
        parts = rel.split("/")
...
def process_markdown_files() -> dict[str, Any]:
    """全量 ingest：按 relativePath 删旧再批量嵌入插入（与 processMarkdownFiles 一致）。"""
    ...
    for i in range(0, len(chunks), EMBED_BATCH_SIZE):
        ...
        if DEBUG_INGEST:
            print(
                f"[ingest] Embedded {min(i + EMBED_BATCH_SIZE, len(chunks))}/{len(chunks)} chunks"
            )
...
def sync_content_to_vector() -> dict[str, Any]:
    """同步：按 slug 删旧；逐块 getEmbedding（与 syncContentToVector 一致）。"""
    chunks = get_all_markdown_chunks()
    unique_paths = sorted({c.metadata.relative_path for c in chunks})
    files_scanned = len(unique_paths)
    if not chunks:
...
# --- 内存任务队列（与 app/api/admin/sync/route.ts 一致，单实例） ---
