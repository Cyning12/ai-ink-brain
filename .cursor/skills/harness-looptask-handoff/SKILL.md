...
- 50 已落盘 `docs/tasks/reinspect_results/` 或维护者索要 **50 Prompt / 签收文档**
...
- **规范真值**：`docs/tasks/specs/PROMPT_50_invoke_<task>_<variant>_zh.md`（若已链出）
- **快照**：`docs/harness/invokes/by-task/<task_slug>/invoke_YYYYMMDD_50_<task_slug>.md`
...
|----|----------------------------------|
| 22 R1 | `docs/harness/reviews/task_<slug>_audit_R1_YYYYMMDD.md` |
| 22 R2 | `docs/harness/reviews/task_<slug>_audit_R2_YYYYMMDD.md` |
| 40 | task 内 **`### 自检结论（执行者）`** |
...
| 项 | 路径 |
|----|------|
| 清单真值 | `docs/tasks/reinspect_results/CHECKLIST_<task_basename>_acceptance_zh.md` |
| 模板 | `docs/tasks/templates/CHECKLIST_TEMPLATE_acceptance_zh.md` |
| SPEC | `docs/tasks/specs/SPEC-harness_acceptance_checklist_v1_zh.md` |
...
4. **归档** → `docs/tasks/README.md`：`git mv docs/tasks/active/<task>.md docs/tasks/done/`，更新 `docs/tasks/_views/done.md`（若项目要求）  
...
- Prompt 规范：`docs/tasks/specs/PROMPT_50_invoke_portfolio_content_sync_w5_v1_zh.md`
- 50 invoke：`docs/harness/invokes/by-task/portfolio-content-sync-v1/invoke_20260601_50_portfolio-content-sync-v1.md`
- R1：`docs/harness/reviews/task_portfolio_content_sync_v1_audit_R1_20260601.md`
- R2：`docs/harness/reviews/task_portfolio_content_sync_v1_audit_R2_20260601.md`
- 50 结论：`docs/tasks/reinspect_results/task_portfolio_content_sync_v1_reinspect_20260601.md`
...
| 步骤 | 文件 | 位置 | 改什么 |
|------|------|------|--------|
| 1 | `docs/tasks/active/task_portfolio_content_sync_script_v1.md` | 文首表 **`### 人工闸 human_gate`** · 行 **`HG-REINSPECT`** · 列 **`status`** | `pending` → **`approved`** |
| 2 | 同上 | **`### KPI（00）`** 整节 | 删除占位「（占位 · CLOSE 后删除）」；按 KPI_RUBRIC_v1_2 填写（CLOSE 帽） |
| 3 | 同上 | 元信息 **`> 状态`** 行 | 关账后改为 **`done`**（可选，与 `git mv` 同步） |
| 4 | — | Git | `git mv docs/tasks/active/task_portfolio_content_sync_script_v1.md docs/tasks/done/task_portfolio_content_sync_script_v1.md`（见 `docs/tasks/README.md`） |
...
1. 30–40 完成后由 22 R2 链出 `docs/tasks/specs/PROMPT_50_invoke_<slug>_v1_zh.md`
