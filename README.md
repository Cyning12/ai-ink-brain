...
**Harness 纪律包**：已接入 `@cyning/harness`（钉版本见仓根 [`harness.pin.json`](harness.pin.json)，当前 **2.19.0** · preset `harness-only`）。过程轨索引：[`docs/harness/README.md`](docs/harness/README.md)。关账须填 `wiki_delta`（缺字段 BLOCK；勿默认 `--allow-*-gap`）。
