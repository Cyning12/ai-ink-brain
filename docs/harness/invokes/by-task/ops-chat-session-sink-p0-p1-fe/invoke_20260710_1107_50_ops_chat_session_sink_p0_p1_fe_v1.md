# 50-independent-reinspect Invoke · ops-chat-session-sink-p0-p1-fe
...
| **prev_invoke** | ai-ink-brain/docs/harness/invokes/by-task/ops-chat-session-sink-p0-p1-fe/invoke_20260710_1107_40_ops_chat_session_sink_p0_p1_fe_v1.md |
| **mode** | 两者 |

## 可复制 Prompt 正文（已替换占位符）
...
1. 读取 task 内「### 自检结论（执行者）」与「### 自检结论（40 复核）」；若缺失 → 阻塞首条：要求先跑 TEMPLATE-self-check-invoke + 40。
