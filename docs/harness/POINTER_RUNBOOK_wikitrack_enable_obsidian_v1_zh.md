# POINTER · WikiTrack 启用 + Obsidian（对照 ops-desk-api）

> **本文件**：从 `Ops-desk/ops-desk-api/docs/harness/RUNBOOK_wikitrack_enable_obsidian_v1_zh.md` **拷贝对照** · Ink 前端仓。  
> **冲突真值**：以本仓 [`docs/harness/README.md`](./README.md) · `harness.pin.json` · preset `harness-only` 为准。  
> **本波（2.19.0）**：WikiTrack **暂不强制启用**（`profile.wiki=false` · 存量 `wiki_delta=n/a`）；维护者明示后再跑棒 B。  
> **Obsidian**：若启用，vault **只开** `docs/coding_wiki/`（禁止开上级 `docs/`）。
