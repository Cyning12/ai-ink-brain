...
> **落盘**：invoke → `docs/harness/invokes/by-task/portfolio-demo-site-epic/`；50 结论 → `docs/tasks/reinspect_results/`。
...
| 占位符 | 含义 | 本 Epic 默认值 |
| --- | --- | --- |
| `{{TASK_PATH}}` | Epic task（相对 `Projects/`） | `ai-ink-brain/docs/tasks/active/task_portfolio_demo_site_epic_v1.md` |
...
| `{{AUDIT_REVIEW_PATH}}` | 22 终轮签收 | `docs/harness/reviews/task_portfolio_demo_site_epic_v1_audit_R2_YYYYMMDD.md`（R2 落盘后填真实文件名；派发前无则 `无` 且 50 须声明风险） |
| `{{FREEZE_ID}}` | task 声明 | `PORTFOLIO-DEMO-SITE@2026-06-09`（与 SPEC 元信息一致；若 task 写后端同源 id 以 task 为准） |
| `{{SPEC_PATH}}` | 冻结 SPEC | `docs/tasks/specs/SPEC-portfolio_demo_site_v1_zh.md` |
...
| 写 `docs/tasks/reinspect_results/task_portfolio_demo_site_epic_v1_reinspect_*.md` | 创建/改 task 验收勾选（仅建议） |
...
- 模式：{{REINSPECT_MODE}}
- 落盘：
  - reinspect → docs/tasks/reinspect_results/task_portfolio_demo_site_epic_v1_reinspect_YYYYMMDD.md
  - invoke 若缺 → docs/harness/invokes/by-task/portfolio-demo-site-epic/
...
- docs/tasks/specs/PROMPT_50_invoke_portfolio_demo_site_v1_zh.md（本 Epic 边界）
...
0. **Invoke 快照**：若 `docs/harness/invokes/by-task/portfolio-demo-site-epic/` 下无本次 50 快照，先将 **本消息全文** 落盘 `invoke_YYYYMMDD_50_portfolio-demo-site-epic.md` 后 commit（同一会话追问不重复落盘）。
...
- `docs/tasks/reinspect_results/task_portfolio_demo_site_epic_v1_reinspect_YYYYMMDD.md`
...
3. 勾选 task 验收；`git mv` → `docs/tasks/done/`（若 policy 允许且 HG-AUDIT-CLOSE 已 approved）  
