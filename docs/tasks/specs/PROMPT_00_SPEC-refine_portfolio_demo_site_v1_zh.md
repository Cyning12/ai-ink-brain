...
> **用途**：在 **冻结 `freeze_id` / 创建 `docs/tasks/active/task_*.md` 之前**，对 [`SPEC-portfolio_demo_site_v1_zh.md`](./SPEC-portfolio_demo_site_v1_zh.md) 做多轮 **人-Agent 对齐**；每轮固定三步：**读 → 问 → 改 SPEC**。  
...
| 占位符 | 含义 | 本 Epic 默认值 |
| --- | --- | --- |
| `{{SPEC_PATH}}` | 目标 SPEC | `docs/tasks/specs/SPEC-portfolio_demo_site_v1_zh.md` |
...
- 修改 `app/`、`components/`、`lib/`、`docs/harness/invokes/`
...
- docs/tasks/specs/PROMPT_00_SPEC-refine_portfolio_demo_site_v1_zh.md
...
docs/tasks/specs/SPEC-portfolio_demo_site_v1_zh.md
...
- docs/tasks/specs/SPEC-portfolio_demo_site_v1_zh.md（**active** · 只读真值）
- docs/tasks/specs/投递冲刺_20260609_v1_zh.md §2（五问 chip 真值）
...
- ai-ink-brain/docs/tasks/templates/TASK_TEMPLATE.md
...
2. 按 **10 需求帽** 在 `docs/tasks/active/` 创建 **首个** task（建议从 **W1** `task_portfolio_site_mode_nav_v1` 起，或人指定的 Wn）。
...
- `docs/tasks/active/task_*.md` 路径（1 个或多个草案，至少 W1）
...
| 10 / 22 / 30 / 40 | 主会话 · `semi_auto` | `docs/harness/invokes/by-task/portfolio-demo-site-epic/` |
| **50** | **`Task` 子 Agent 专收** | 同上 + `docs/tasks/reinspect_results/` |
| CLOSE | 主会话 | — |
...
| **文件名** | `docs/tasks/active/task_portfolio_demo_site_epic_v1.md` |
| **semi_auto** | `true` |
