...
> **类型**：规格初稿（`docs/tasks/specs/`）；实施时拆为 `docs/tasks/active/task_*.md`  
...
- 前端仓 **方案1** 已关闭：`docs/tasks/done/task_engineering_tech_graph_graph_json_export_v1.md`（`quality` 内 `tech_graph_graph_export.py --check`，复用后端脚本 + 双 checkout）。
...
**迁移实践（Quickstart 种子）**：`docs/tasks/specs/MIGRATION-tech_graph_v2_frontend_playbook_v1_zh.md`。
...
| 前端已关闭 scheme1 | `docs/tasks/done/task_engineering_tech_graph_graph_json_export_v1.md` |
...
- 子 task 拆分、验收表、Harness 字段（`test_strategy`、`failure_paths`、`freeze_id`）按 `docs/tasks/README.md` 与根 `AGENTS.md` §2.3。
...
`graph_v2`、`docs/_tech_graph`、`parity`、`equivalence`、`quality.yml`、`tech_graph_graph_export`、`freeze_id`、`SPEC-tech_graph_v2_frontend_parity_v1`、`docs/tasks/specs`
