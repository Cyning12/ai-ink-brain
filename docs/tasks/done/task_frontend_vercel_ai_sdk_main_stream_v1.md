...
> **执行帽 invoke（30 · PR3）**：`docs/harness/invokes/by-task/frontend-vercel-ai-sdk-main-stream/invoke_20260521_30_frontend-vercel-ai-sdk-pr3-timeline.mdby-task/frontend-vercel-ai-sdk-main-stream/invoke_20260521_30_frontend-vercel-ai-sdk-pr3-timeline.md`  
> **执行帽 invoke（30）**：`docs/harness/invokes/by-task/frontend-vercel-ai-sdk-main-stream/invoke_20260520_30_frontend-vercel-ai-sdk-main-stream-execute.mdby-task/frontend-vercel-ai-sdk-main-stream/invoke_20260520_30_frontend-vercel-ai-sdk-main-stream-execute.md`  
> **登记日期**：2026-05-20  
> **需求帽回填**：2026-05-20  
> **任务审核 R1**：`docs/harness/reviews/task_frontend_vercel_ai_sdk_main_stream_v1_audit_R1_20260520.md`（工作区指针：`docs/harness/reviews/pointer_task_frontend_vercel_ai_sdk_main_stream_v1_audit_R1_20260520.md`）  
> **分支建议**：`feat/unified-chat-ai-sdk-stream-v1`（自 `main`；当前文档分支 `docs/vercel-ai-sdk-streaming-backlog-v1` 仅含 Harness 落盘）  
> **需求帽 invoke**：`docs/harness/invokes/by-task/frontend-vercel-ai-sdk-main-stream/invoke_20260520_10_frontend-vercel-ai-sdk-main-stream-requirements.md`（**未入库** · 审查全文为准）  
> **审核帽 invoke**：`docs/harness/invokes/by-task/frontend-vercel-ai-sdk-main-stream/invoke_20260520_22_frontend-vercel-ai-sdk-main-stream-audit.md`（**未入库** · 审查全文为准）  
...
- [ ] `docs/tasks/active/` 或 `docs/diary/` 附 **3 分钟演示脚本**（见 §可观测验收 · 演示）。
...
| v1 已验收基线 | `docs/tasks/done/task_frontend_unified_chat_streaming_sse_v1.md` |
| vNext Timeline UI | `docs/tasks/active/task_chatbi_v2_incremental_sse_timeline_frontend_v1.md` |
...
1. 先读 **done** `docs/tasks/done/task_frontend_unified_chat_streaming_sse_v1.md` 与现网 `UnifiedChatPageClient.tsx` **993–1200 行** 附近 SSE 循环（迁出基准）。  
...
| 执行帽 invoke | `docs/harness/invokes/by-task/frontend-vercel-ai-sdk-main-stream/invoke_20260521_30_frontend-vercel-ai-sdk-pr3-timeline.mdby-task/frontend-vercel-ai-sdk-main-stream/invoke_20260521_30_frontend-vercel-ai-sdk-pr3-timeline.md` |
| `UnifiedChatPageClient.tsx` 行数 | **923**（目标 ≤1200；SSE 逻辑 100% 在 `lib/unified-chat/`；页面无 `ReadableStream`/`parseSseBlocks`） |
| 上一棒 PR2 自检 | `docs/harness/invokes/by-task/frontend-vercel-ai-sdk-main-stream/invoke_20260520_40_frontend-vercel-ai-sdk-main-stream-self-check-r3.mdby-task/frontend-vercel-ai-sdk-main-stream/invoke_20260520_40_frontend-vercel-ai-sdk-main-stream-self-check-r3.md` |
...
| `pnpm test` | `ai-ink-brain` | **0** | **9** files / **32** tests passed |
| `pnpm exec vitest run lib/unified-chat/sse lib/unified-chat/transport` | `ai-ink-brain` | **0** | **3** files / **12** tests；`ChatbiSseTransport` 断言 `X-ChatBI-Sse-Contract: 2`；abort 后无后续 `text-delta` |
| `pnpm build` | `ai-ink-brain` | **0** | Next **16.2.3** webpack；TS + 135 静态页生成通过 |

#### 验收项（§8 母单 · PR3 相关）

| 条目 | 结果 | 证据 |
| --- | --- | --- |
| **契约不退化** | **pass** | `chatbiSseTransport.test.ts` 捕获 `CHATBI_SSE_CONTRACT_HEADER` = `2`；BFF `route.ts` 仍透传 `x-chatbi-sse-contract` |
| **Timeline** | **pass（CI + 结构）** | `useUnifiedChatStream` → `timelineEvents` → `ChainTimeline`；`chainRoundState` / `executionTrace` vitest 绿 |
| **中止** | **pass（单测）** / **人测未复跑** | `createChatbiSseUiMessageStream` · `emits no text-delta after abort`；UI「停止」见 §9 阻塞 |
| **结构** | **pass** | `UnifiedChatPageClient.tsx` **923** 行；SSE 读流仅在 `lib/unified-chat/transport/chatbiSseStream.ts` |
| **演示路径**（3 分钟稳定出字） | **未测** | T4 脚本仍 open；本帽无浏览器会话 |
| **文档**（§实现备忘） | **partial** | PR3 路径/行数已回填；演示录屏 **待 PR4** |
...
| V-TRANSPORT | **pass** | vitest `lib/unified-chat/transport`（≥2 `text-delta`；abort 截断） |
| V-NET | **pass（单测）** / **人测阻塞** | 单测：`ChatbiSseTransport` 契约头；人测：`curl localhost:3000/unified-chat` → 无服务（**000**） |
| V-RAG | **阻塞（人测）** | 需 `pnpm dev` + Python `PY_API_URL` + 登录/Token；**继承** PR2 维护者签收（2026-05-18） |
| V-SQL | **阻塞（人测）** | 同上；text2sql 环境依赖 |
| V-DONE-FAIL | **阻塞（人测）** | 同上 |
| V-ABORT | **阻塞（人测）** | 同上；abort **单测**已 pass |

#### PR3 结论（40 帽）

- **PR3 → `main`**：**建议合并**（CI 子集全绿；T3 瘦身达标；契约/abort 有自动化证据）。  
- **母单关闭**：**否** — T4 演示脚本、§8「3 分钟演示」、§9 人测五项须合并前由维护者复跑或 PR4 签收。  
- **下一棒**：**50 复检**（diff `main..feat/unified-chat-ai-sdk-stream-v1` @ `62e85fb`）或维护者补人测后更新 §9。
...
- **50 独立复检**：`docs/harness/reviews/task_frontend_vercel_ai_sdk_main_stream_v1_reinspect_R1_20260520.md`（`393f877..906a062`）。
