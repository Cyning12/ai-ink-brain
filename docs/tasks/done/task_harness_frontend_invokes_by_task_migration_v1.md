...
> **关联**：`[docs/harness/invokes/MIGRATION_flat_invoke_index.md](../../harness/invokes/MIGRATION_flat_invoke_index.md)` · 后端 `[docs/harness/README.md](../../../../ai-ink-brain-api-python/docs/harness/README.md)` §2.1  
...
前端 `docs/harness/invokes/` 根目录遗留 **17 个** P1-4 前扁平 `invoke_*.md`，与后端已完成的 `by-task/<task_slug>/` taxonomy 不一致，导致 Agent 续跑时仍可能落盘到根目录。

**完成态**：

1. 根目录 **无** 新增/遗留 `invoke_*.md`（仅 `README.md` + `MIGRATION_flat_invoke_index.md` + `by-task/`）
2. 全部历史 invoke 按 slug 归入 `by-task/`
3. `reviews/`、`docs/tasks/done/` 内链接已更新
...
- 更新 `docs/harness/reviews/` 中 5 份 review 链接
- 更新 `docs/tasks/done/` 相关 task 路径
- `rg` 验收：无 `docs/harness/invokes/invoke`_ 残留（本索引 / task 正文除外）
- 更新 `docs/harness/invokes/README.md` · `by-task/README.md`
- 更新 `docs/harness/README.md` §2

## 非范围

- 不迁移 `docs/harness/reviews/` 至 `reviews/by-task/`（可选 P2）
...
```bash
mkdir -p docs/harness/invokes/by-task/frontend-vercel-ai-sdk-main-stream
git mv docs/harness/invokes/invoke_20260520_30_frontend-vercel-ai-sdk-main-stream-execute.md \
       docs/harness/invokes/by-task/frontend-vercel-ai-sdk-main-stream/
# … 对其余 16 个文件重复（见索引表）
```
...
rg 'docs/harness/invokes/invoke_' content/ --glob '*.md'
rg '\.\./invokes/invoke_' docs/harness/reviews/
```
...
- `docs/harness/invokes/README.md`：删除「历史根目录只读」表述，改为「根目录禁止新增 invoke」
- `docs/harness/invokes/by-task/README.md`：补全 5 个迁移 slug 表
- `docs/harness/README.md` §2：注明迁移完成日期

### Phase 4 · 验收

```bash
ls docs/harness/invokes/invoke_*.md 2>/dev/null | wc -l   # 期望 0
find docs/harness/invokes/by-task -name 'invoke_*.md' | wc -l  # 期望 ≥31（14 原有 + 17 迁移）
