...
`docs/tasks/done/task_chatbi_v3_multiturn_clarify_semantics_4_3_frontend_v1.md`（相对该仓根）。

| 目标 | 写法 |
|------|------|
| **同仓 `docs/tasks/done/`** | Markdown 可用 [`../done/…`](../done/task_chatbi_v3_text2sql_phase_sse_timeline_frontend_v1.md)（从 `active/` 上一级到 `tasks/` 再进 `done/`） |
| **兄弟仓 `ai-ink-brain-api-python`（与总仓 AGENTS 一致：两仓均为 `Projects/` 下同级目录）** | **推荐**：在文档/PR 中写 **`Projects/ai-ink-brain-api-python/docs/...`** 全路径，便于人与 Cursor `@` 引用。**勿**使用 `../ai-ink-brain-api-python/` 从本文件出发——在磁盘上会解析到 **`docs/tasks/ai-ink-brain-api-python/`**，该路径**不存在**。 |
...
| 决议 | 说明 |
|------|------|
| **v1 关单** | **§验收标准** 全部 `- [x]` → 状态 `done（YYYY-MM-DD）` → `git mv` → `docs/tasks/done/`（见 `docs/tasks/README.md`）。 |
