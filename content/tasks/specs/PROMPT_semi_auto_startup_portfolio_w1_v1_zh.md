...
| 帽 | 执行者 | invoke 落盘 |
| --- | --- | --- |
| 30 / 40 / 22 | **本会话** · semi_auto | `content/harness/invokes/by-task/portfolio-site-mode-nav-v1/` |
| **50** | **`Task` 子 Agent**（本会话禁止兼做） | 同上 + `content/tasks/reinspect_results/` |
...
- ai-ink-brain/content/tasks/specs/PROMPT_50_invoke_portfolio_site_mode_nav_w1_v1_zh.md（50 · W1 真值 · Task 子代理）
...
1. **换帽前**：下一棒 TEMPLATE §3 全文 → `content/harness/invokes/by-task/portfolio-site-mode-nav-v1/invoke_YYYYMMDD_<帽号>_*.md` → **commit** → 再戴帽执行。
...
- task：ai-ink-brain/content/tasks/active/task_portfolio_site_mode_nav_v1.md
- SPEC：ai-ink-brain/content/tasks/specs/SPEC-portfolio_demo_site_v1_zh.md
...
- 落盘：`content/harness/reviews/task_portfolio_site_mode_nav_v1_audit_R2_20260601.md`（日期用当日）
...
**真值 Prompt（已填占位符）**：[`content/tasks/specs/PROMPT_50_invoke_portfolio_site_mode_nav_w1_v1_zh.md`](./PROMPT_50_invoke_portfolio_site_mode_nav_w1_v1_zh.md) · §4 Handoff + §5 全文  
**invoke 落盘**：`content/harness/invokes/by-task/portfolio-site-mode-nav-v1/invoke_20260601_50_portfolio-site-mode-nav-v1.md`  
...
- 勾选 task 验收项；`git mv` → `content/tasks/done/task_portfolio_site_mode_nav_v1.md`（**须**人后续改 HG-REINSPECT approved 再 merge，或本 Prompt 仅建议关账路径）
