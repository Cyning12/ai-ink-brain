...
- [ ] **`OpsSessionPromotePanel`**（或向导步骤）：预览清单 · target_repo · target_branch · 确认按钮
- [ ] **BFF**：`GET/POST /api/ops/sessions/[id]/promote`（及 preview 若拆分）· `forwardOpsRequest`
- [ ] **verify 报告展示**：只读 JSON 摘要 / 失败项列表
- [ ] **gate 展示**：`HG-PROMOTE` pending/approved 与 session 详情同步
- [ ] **manifest / tech-graph**：新 BFF 路由登记
- [ ] **Vitest**：promote BFF smoke（mock forward）
- [ ] **Session 详情**：`dispatched` 且无阻塞时展示 promote 入口
...
- [ ] promote 预览与确认流与 API 契约一致
- [ ] verify 失败/成功态 UI 可辨 · 不静默失败
- [ ] S2/S3 授权 · 深析 · deliverables **不退化**
- [ ] `pnpm lint` / `test` / `build` 绿

---

## 给 Cursor

`ops-session-s4-verify-ui` · Open `ai-ink-brain/` · **HG-AUDIT-R1 pending 拒开工** · PR 于 S4 本地验收后与 S2–S4 合并批次
