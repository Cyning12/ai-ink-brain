...
"""
    ...
    @staticmethod
    def _plan_intent_heuristic(query: str) -> tuple[bool, str, float]:
        """
        方案触发器（启发式）：判断用户是否在寻求“建议/方案/攻略/怎么玩/怎么办”。
        返回：(need_plan, reason, confidence)。
        """
        q = (query or "").strip()
        if not q:
            return False, "空查询", 1.0
        cues = (
            "怎么办",
            "怎么做",
            "怎么解决",
        ...
        for c in cues:
            if c in q:
                return True, f"hit:{c}", 0.85
        return False, "no_plan_cue", 0.7

    def identify_need_plan(self, query: str) -> dict:
        """识别用户是否在寻求建议/方案/攻略（先用启发式，后续可替换为 LLM 分类）。"""
        need, reason, cf = self._plan_intent_heuristic(query)
        return {
            "need_plan": bool(need),
            "plan_reason": str(reason),
            "confidence": float(cf),
        }

    def hyde_solution_keywords(self, query: str) -> list[str]:
        """
        HyDE（Query2Doc 前置）：不直接改写 query，而是先生成“可能的解决方案关键词”。

        模板要求（用户指定）：
        “如果你是迪士尼资深导游，请针对‘{query}’给出三个可能的解决方案关键词。”
        """
        q = (query or "").strip()
        if not q:
            return []
        system = (
            "你是迪士尼资深导游。\n"
            "任务：针对用户问题，给出三个“可能的解决方案关键词”。\n"
            "输出要求：只输出严格 JSON 数组（长度=3），不要解释文字。\n"
            '示例：["关键词1", "关键词2", "关键词3"]\n'
        )
        user = f"用户问题：{q}\n请输出三个解决方案关键词（JSON数组）。"
        arr = self._call_rewrite_model(
            system=system, user=user, output_format="json_array"
        )
        out: list[str] = []
        for x in arr:
    ...
    def hyde_expand_to_doc(self, query: str, keywords: list[str]) -> str:
        """
        HyDE 的 Query2Doc：用**不带联网工具**的 LLM 先扩写一个“假设文档”（用于后续检索/搜索提示）。
        注意：这里不要求事实准确，强调“作为搜索与规划的草案”，避免模型胡编当真。
        """
        q = (query or "").strip()
        if not q:
            return ""
        ...
        system = (
            "你是一个行程规划草案生成器。\n"
            "你将基于用户问题与关键词，生成一段“可用于后续联网核验/搜索”的规划草案。\n"
            "规则：\n"
            "1) 不要编造具体票价/日期/活动名称等需要实时核验的数据。\n"
            "2) 用要点列出 3 个方案方向，每个方向给出可搜索的关键短语。\n"
            "3) 输出为纯文本，不要 JSON。\n"
        )
        user = (
            f"用户问题：{q}\n"
            f"关键词：{kws}\n\n"
            "请输出规划草案（包含3个方案方向 + 每个方向的可搜索短语）。"
        )
        resp = call_generation(
            model=self._model,
            api_key=self._api_key,
    ...
    def plan_with_hyde_then_search_answer(
        self,
        *,
        query: str,
        conversation_history: str = "",
        ...
        """
        完整 HyDE 应用场景：
        - 方案触发器 → HyDE 关键词（3个）→ Query2Doc 扩写（无搜索 LLM）→
          使用“可搜索的 LLM（Function Calling）”联网核验并输出最终回答。
        """
        q = (query or "").strip()
        if not q:
            return {"ok": False, "error": "empty_query"}

        plan = self.identify_need_plan(q)
        ...
        kws = self.hyde_solution_keywords(q)
        doc = self.hyde_expand_to_doc(q, kws)

        system = (
            "你是一个带联网工具的智能助手。\n"
            "你会收到：用户问题、对话历史/上下文、HyDE 关键词、以及一份规划草案（可能不准确）。\n"
            "当涉及最新/实时/票价/活动/开放时间等外部信息时，你必须调用 web_search 工具核验。\n"
            "回答要求：\n"
            "1) 先给结论，再给3个可执行方案。\n"
            "2) 对需要核验的数据给出来源链接（URL 或编号）。\n"
            "3) 说明哪些信息来自联网核验，哪些是通用建议。\n"
        )
        user = (
            f"【用户问题】\n{q}\n\n"
            f"【对话历史】\n{conversation_history}\n\n"
            ...
            f"【HyDE 关键词（3个）】\n{kws}\n\n"
            f"【规划草案（待核验）】\n{doc}\n\n"
            "请基于以上信息回答；需要事实/时效信息请先 web_search 再回答。"
        )
        m = (
            (search_model or "").strip()
...
def auto_web_search_rewrite_demo_with_search_function_call():
    ...
    # 从 trace 中提取本次工具调用的搜索 query（改写后的 query）
    search_query_used = ""
    try:
        for m in trace:
            ...
            if isinstance(fc, dict) and (fc.get("name") == "web_search"):
                args_raw = fc.get("arguments") or ""
                if isinstance(args_raw, str) and args_raw.strip():
                    args = json.loads(args_raw)
                    search_query_used = str(args.get("query") or "").strip()
                    break
            tc = m.get("tool_calls")
            if isinstance(tc, list) and tc:
                ...
                if first:
                    ...
                    if isinstance(fn, dict) and fn.get("name") == "web_search":
    ...
    if search_query_used:
        print(f"- 搜索query（改写后）: {search_query_used}")
    print(f"- trace_messages: {len(trace)}")
    print("---- Answer ----")
    print(chat_answer_text(resp))


def hyde_plan_trigger_demo() -> None:
    """Demo：方案触发器 + HyDE（关键词→Query2Doc）+ 可搜索 LLM 最终回答。"""
    logging.basicConfig(
        level=logging.INFO, format="%(levelname)s %(name)s: %(message)s"
    )
    qr = QueryRewrite()
    query = "第一次带娃去上海迪士尼怎么玩比较省心？有什么推荐的行程方案？"
    result = qr.plan_with_hyde_then_search_answer(query=query)
    print("\n=== hyde_plan_trigger_demo ===")
    print(f"- 原始问题: {query}")
    if not result.get("need_plan"):
        print("✗ 未触发方案模式")
        print(f"  原因: {result.get('reason')}")
        return
    print("✓ 已触发方案模式（HyDE）")
    print(f"- reason: {result.get('plan_reason')}")
    print(f"- hyde_keywords: {result.get('hyde_keywords')}")
    print(f"- trace_messages: {result.get('trace_messages')}")
    print("---- Answer ----")
